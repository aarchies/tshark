...
bool test_for_regular_file(const char *path)
{
    ws_statb64 statb;
...
configuration_init_posix(const char *arg0)
{
    ...
    ws_debug("curent working directory: %s", get_current_executable_path());
    ...
    else
    {
        ...
        if (path_start != NULL)
        {
            while (*path_start != '\0')
            {
                ...
                path_len = path_component_len + 1 + strlen(execname) + 1;
...
bool profile_exists(const char *profilename, bool global)
{
    char *path = NULL;
    bool exists;
...
int delete_persconffile_profile(const char *profilename, char **pf_dir_path_return)
{
 ...
 * return 0.
 */
int create_persconffile_profile(const char *profilename, char **pf_dir_path_return)
{
    char *pf_dir_path;
#ifdef _WIN32
    ...
    if (ws_stat64(pf_dir_path, &s_buf) != 0)
    {
        ...
        if (pf_dir_parent_path_len > 0 && pf_dir_parent_path[pf_dir_parent_path_len - 1] != ':' && ws_stat64(pf_dir_parent_path, &s_buf) != 0)
        {
            /*
...
int copy_persconffile_profile(const char *toname, const char *fromname, bool from_global,
 ...
 */
bool files_identical(const char *fname1, const char *fname2)
{
...
bool file_needs_reopen(int fd, const char *filename)
{
#ifdef _WIN32
...
bool write_file_binary_mode(const char *filename, const void *content, size_t content_len)
{
    int fd;
    size_t bytes_left;
 ...
 */
bool copy_file_binary_mode(const char *from_filename, const char *to_filename)
{
