...
try_open(wtap *wth, unsigned int type, int *err, char **err_info)
{
	...
	if (extension != NULL)
	{
		...
		for (pass = 0; pass < 3 && result == WTAP_OPEN_NOT_MINE; pass++)
		{
			...
			{
				if ((pass == 0 && heuristic_uses_extension(i, extension)) || (pass == 1 && open_routines[i].extensions == NULL) || (pass == 2 && open_routines[i].extensions != NULL && !heuristic_uses_extension(i, extension)))
				{
					result = try_one_open(wth, &open_routines[i], err, err_info);
				}
			}
...
int try_file(const char *path, int *err)
{
	int nb_read;
	char buf[1024];
	int fd = ws_open(path, O_BINARY | O_RDWR, 0644);
	if (fd == -1)
	{
		goto fail;
	}
...
wtap *
open_wtap_from_binary(const char *path, const char *binary, size_t len, int *err, char **err_info)
{
	int fd = ws_open(path, O_BINARY | O_CREAT | O_WRONLY, 0644);

	if (write(fd, binary, (int)len) != (ssize_t)len && lseek(fd, 0, SEEK_SET) == -1)
	{
		goto fail;
	};
 ...
 */
wtap *
wtap_open_offline(const char *filename, unsigned int type, int *err, char **err_info, bool do_random)
{
	int fd;
	ws_statb64 statb;
	bool ispipe = false;
...
bool wtap_dump_can_write_encap(int file_type_subtype, int encap)
{
	...
	if (result != 0)
	{
		...
		if (result == WTAP_ERR_CHECK_WSLUA && file_type_subtype_table[file_type_subtype].wslua_info != NULL && file_type_subtype_table[file_type_subtype].wslua_info->wslua_can_write_encap != NULL)
		{
 ...
 */
bool wtap_dump_can_open(int file_type_subtype)
{
	if (file_type_subtype < 0 ||
		file_type_subtype >= (int)file_type_subtype_table_arr->len ||
...
#else
bool wtap_dump_can_compress(int file_type_subtype _U_)
{
	return false;
}
#endif

static bool wtap_dump_open_finish(wtap_dumper *wdh, int *err, char **err_info);
// bool wtap_dump_file_write(wtap_dumper *wdh, const void *buf, size_t bufsize, int *err);
					  ...
					  const wtap_dump_params *params, int *err)
{
	...
	*err = (*file_type_subtype_table[file_type_subtype].can_write_encap)(params->encap);
	/* if the err said to check wslua's can_write_encap, try that */
	if (*err == WTAP_ERR_CHECK_WSLUA && file_type_subtype_table[file_type_subtype].wslua_info != NULL && file_type_subtype_table[file_type_subtype].wslua_info->wslua_can_write_encap != NULL)
	{
...
bool wtap_dump(wtap_dumper *wdh, const wtap_rec *rec,
...
void wtap_dump_discard_sysdig_meta_events(wtap_dumper *wdh)
{
	/* As above for DSBs. */
	if (wdh->mevs_growing)
	{
