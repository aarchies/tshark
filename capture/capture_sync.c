...
#endif
{
    ...
    *message_read_io = NULL;
    ...
    {
        if (i != 0)
            g_string_append_c(args, ' '); /* don't prepend a space before the path!!! */
...
bool sync_pipe_start(capture_options *capture_opts, GPtrArray *capture_comments,
    ...
    for (j = 0; j < capture_opts->ifaces->len; j++)
    {
        ...
        if (interface_opts->sampling_method != CAPTURE_SAMP_NONE)
        {
            ...
            snprintf(ssampling, ARGV_NUMBER_LEN, "%s:%d",
                     interface_opts->sampling_method == CAPTURE_SAMP_BY_COUNT ? "count" : interface_opts->sampling_method == CAPTURE_SAMP_BY_TIMER ? "timer"
                                                                                                                                                   : "undef",
...
int sync_interface_set_80211_chan(const char *iface, const char *freq, const char *type,
 ...
 * must be freed with g_free().
 */
int sync_interface_list_open(char **data, char **primary_msg,
                             char **secondary_msg, void (*update_cb)(void))
{
    int argc;
    char **argv;
 ...
 * must be freed with g_free().
 */
int sync_if_capabilities_open(const char *ifname, bool monitor_mode, const char *auth,
                              char **data, char **primary_msg,
                              char **secondary_msg, void (*update_cb)(void))
{
    int argc;
...
int sync_if_list_capabilities_open(GList *if_queries,
                                   char **data, char **primary_msg,
                                   char **secondary_msg, void (*update_cb)(void))
{
    int argc;
 ...
 */
int sync_interface_stats_open(int *data_read_fd, ws_process_id *fork_child, char **data, char **msg, void (*update_cb)(void))
{
    int argc;
    char **argv;
    int ret;
